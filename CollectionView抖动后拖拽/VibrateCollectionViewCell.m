...
- (void)CreateUI{
    ...
    if (!_delectedBtn) {
        _delectedBtn = [UIButton buttonWithType:0];
        _delectedBtn.frame = CGRectMake(114*kWidth/750.00/2, 114*kWidth/750.00/2, 20, 20);
        [_delectedBtn setTitle:@"-" forState:0];
        [_delectedBtn setBackgroundColor:[UIColor redColor]];
        [_delectedBtn addTarget:self action:@selector(test) forControlEvents:UIControlEventTouchUpInside];
        [_headImageView addSubview:_delectedBtn];
...
- (void)test{
    if ([self respondsToSelector:@selector(test)]) {
        [self.delegate clickdelected:self.delectedBtn];
    }
}
